...
export const getAccountBlocked = (req, res) => {
...
export const postLogin = async (req, res) => {
  try {
    ...
    if (!result.success) {
      return res.status(HTTP_STATUS_CODES.BAD_REQUEST).json({ success: false, message: result.errors?.general || 'Login failed', errors: result.errors });
    }

    req.session.user = { id: result.user._id, name: result.user.name, email: result.user.email };
    return res.status(HTTP_STATUS_CODES.OK).json({ success: true, message: 'Logged in successfully.', data: { redirectUrl: '/user/dashboard' } });
  } catch (err) {
    console.log(err);
    return res.status(HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR).json({ success: false, message: 'Something went wrong', data: {} });
  }
};
...
export const postSignup = async (req, res) => {
  try {
    ...
    if (!result.success) {
      return res.status(HTTP_STATUS_CODES.BAD_REQUEST).json({ success: false, message: result.errors?.general || 'Signup failed', errors: result.errors });
    }
    ...
    return res.status(HTTP_STATUS_CODES.OK).json({ success: true, message: 'OTP sent successfully.', data: { redirectUrl: '/auth/verify-signupotp' } });
  } catch (err) {
    console.log(err);
    return res.status(HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR).json({ success: false, message: 'Something went wrong', data: {} });
  }
...
export const getVerifySignupOtp = (req, res) => {
...
export const postVerifySignupOtp = async (req, res) => {
  try {
    ...
    if (!result.success) {
      return res.status(HTTP_STATUS_CODES.BAD_REQUEST).json({ success: false, message: result.errors?.general || 'OTP verification failed', errors: result.errors });
    }
    ...
    req.session.save((err) => {
      ...
      return res.status(HTTP_STATUS_CODES.OK).json({ success: true, message: 'Account created successfully.', data: { redirectUrl: '/auth/login' } });
    });
  } catch (err) {
    console.log(err);
    return res.status(HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR).json({ success: false, message: 'Something went wrong', data: {} });
...
export const resendSignupOtp = async (req, res) => {
  try {
    if (!req.session.signupData) {
      return res.status(HTTP_STATUS_CODES.BAD_REQUEST).json({ success: false, message: 'Session expired. Please signup again.', data: {} });
    }
    const result = await authService.resendSignupOtp(req.session.signupData.email);
    if (!result.success) {
      return res.status(HTTP_STATUS_CODES.BAD_REQUEST).json({ success: false, message: result.errors?.general || 'Failed to resend OTP', errors: result.errors });
    }
    req.session.signupOTP = result.otp;
    req.session.signupOTPExpires = result.otpExpires;
    return res.status(HTTP_STATUS_CODES.OK).json({ success: true, message: 'OTP resent successfully.', data: {} });
  } catch (err) {
    console.log(err);
    return res.status(HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR).json({ success: false, message: 'Failed to resend OTP', data: {} });
  }
};

export const getAccountCreated = (req, res) => {
...
export const postForgotPassword = async (req, res) => {
  try {
    ...
    if (!result.success) {
      return res.status(HTTP_STATUS_CODES.BAD_REQUEST).json({ success: false, message: result.errors?.general || 'Forgot password failed', errors: result.errors });
    }
    req.session.forgotOTP = result.otp;
    req.session.forgotOTPExpires = result.otpExpires;
    req.session.forgotEmail = result.email;
    return res.status(HTTP_STATUS_CODES.OK).json({ success: true, message: 'OTP sent to email.', data: { redirectUrl: '/auth/verify-otp' } });
  } catch (err) {
    console.log(err);
    return res.status(HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR).json({ success: false, message: 'Something went wrong', data: {} });
  }
...
export const postVerifyOtp = async (req, res) => {
  try {
    ...
    req.session.save((err) => {
      if (err) console.error("Session save error:", err);
      return res.status(HTTP_STATUS_CODES.OK).json({ success: true, message: 'OTP verified successfully.', data: { redirectUrl: '/auth/reset-password' } });
    });
  } catch (err) {
    console.log(err);
    return res.status(HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR).json({ success: false, message: 'Something went wrong', data: {} });
...
export const resendOtp = async (req, res) => {
  try {
    if (!req.session.forgotEmail) {
      return res.status(HTTP_STATUS_CODES.BAD_REQUEST).json({ success: false, message: 'Session expired. Please request again.', data: {} });
    }
...
export const postResetPassword = async (req, res) => {
  try {
    if (!req.session.isOtpVerified || !req.session.forgotEmail) {
      return res.status(HTTP_STATUS_CODES.UNAUTHORIZED).json({ success: false, message: 'Unauthorized', data: { redirectUrl: '/auth/forgot-password' } });
    }
    const result = await authService.resetPassword(req.body.password, req.body.confirmPassword, req.session.forgotEmail);
    if (!result.success) {
      return res.status(HTTP_STATUS_CODES.BAD_REQUEST).json({ success: false, message: result.errors?.general || 'Password reset failed', errors: result.errors });
    }
    ...
    req.session.save((err) => {
      if (err) console.error("Session save error:", err);
      return res.status(HTTP_STATUS_CODES.OK).json({ success: true, message: 'Password reset successfully.', data: { redirectUrl: '/auth/login' } });
    });
  } catch (err) {
    console.log(err);
    return res.status(HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR).json({ success: false, message: 'Something went wrong', data: {} });
...
export const getPasswordUpdated = (req, res) => {
...
export const postAdminLogin = async (req, res) => {
  try {
    ...
    req.session.save((err) => {
      if (err) console.error("Session save error:", err);
      return res.status(HTTP_STATUS_CODES.OK).json({ success: true, message: 'Admin logged in successfully.', data: { redirectUrl: '/admin/dashboard' } });
    });
  } catch (err) {
    console.log(err);
    return res.status(HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR).json({ success: false, message: 'Something went wrong', data: {} });
